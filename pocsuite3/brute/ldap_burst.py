    ...
    for user in args.user:
        for passwd in args.passwd:
            try:
                ...
                c = ldap3.Connection(server, user="{domain}\\{user}".format(domain=args.domain, user=user),
...
def main():
    ...
    args.add_argument('--no-stop', dest='no_stop_on_success', action='store_true',
                      help='Do not stop when valid account is found (will stop by default)')
